...
## 4. QALAM markers and derived search indices (app runtime)

- `references/qalam-30-markers.md` — the owner's 30 proposed QALAM markers,
  saved verbatim; see that file's own provenance note. Parsed into
  `prototype/public/data/derived/qalam-markers.json` by
  `prototype/scripts/build-qalam.mjs` — a structural parse (title/anchor/
  body split), never a rewording.
- `prototype/public/data/derived/{root,word}-occurrences*.json`,
  `word-to-root.json`, `vocatives.json` — generated by
  `prototype/scripts/build-index.mjs` directly from `data/quran/words/*.json`
  (the same GPL-licensed morphology as §1). Root/word occurrence loci are a
  complete index (not a sample); the vocative list is a heuristic extraction
  documented in that script's own comments (the "يا" particle fuses
  orthographically with its addressee in this corpus — see the script for
  the exact diacritic signature used to detect it, verified against 0 false
  positives from hamza-initial verbs).
- **Known minor variance:** `word-occurrences-exact.json` (21,287 exact
  surface forms) is 8 entries short of `lexicon/words.json`'s 21,295 —
  not chased further for this build; noted here rather than silently
  rounded off. See `prototype/QA_NOTES.md`.
